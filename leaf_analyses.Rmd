...
## Table S3 and Table 4

Table S3 (snow): first 3 tables. Table 4 (water): next 3 tables.
...
## Selection in each year and treatment

```{r selection_split, eval=F}
selection_split_mod <- function(trait, response, set, .year, .snow, .water, drop=TRUE) { 
  family <- ifelse(response=="RGR", "gaussian", "binomial") 
  t.plantyr <- list(lt = lt.plantyr.r2, pt = pt.plantyr)[[set]] %>% 
    rename("trait"=trait,"response"=response) %>% 
    select(year, snow, water, trait, response, plot) %>% 
    filter(year == .year, snow == .snow, water == .water) %>% 
    drop_na(trait, response)
  options(contrasts = c("contr.sum", "contr.poly"))
  print(paste(response, .year, .snow, .water, trait))
  if(nrow(t.plantyr) == 0) return(NA)
  glm(response ~ trait, data=t.plantyr, family=family)
}
selection.combos <- expand_grid(response = fitnesstraits, set = c("lt", "pt"),
                                .year = levels(treatments$year),
                                .snow = levels(treatments$snow), .water = levels(treatments$water)) 

mod.selection.split <- selection.combos  %>% 
  full_join(tibble(trait=c(leaftraits, phystraits), 
                   set = c(rep("lt", length(leaftraits)), rep("pt", length(phystraits))))) %>% 
  mutate(model = pmap(., selection_split_mod, drop=F),
         coefs = map(model, tidy))

coef.selection.split <- mod.selection.split %>% 
  select(-model) %>% unnest(coefs) %>% filter(term=="trait") %>% 
  mutate(sd.trait=alltraits.sd[trait], estimate.sd = estimate*sd.trait, std.error.sd=std.error*sd.trait) #multiply by SD to get change in fitness per SD

ggplot(coef.selection.split, aes(x=paste(.year, .snow, .water), y=log(estimate.sd))) +
  facet_grid(vars(response), vars(trait)) + geom_point()
```


```{r selection_year, fig.width=12}
selection_year_mod <- function(trait, response, set, .year, drop=TRUE) { 
  family <- ifelse(response=="RGR", "gaussian", "binomial") 
  t.plantyr <- list(lt = lt.plantyr.r2, pt = pt.plantyr)[[set]] %>% 
    rename("trait"=trait,"response"=response) %>% 
    select(year, snow, water, trait, response, plot) %>% 
...
mod.selection.split <- selection.combos  %>% 
  full_join(tibble(trait=c(leaftraits, phystraits), 
                   set = c(rep("lt", length(leaftraits)), rep("pt", length(phystraits))))) %>% 
  mutate(model = pmap(., selection_year_mod, drop=F),
         coefs = map(model, tidy))
...
ggplot(coef.selection.split, aes(x=.year, y=estimate.sd, color=.year,
                                 ymax=estimate.sd+std.error.sd, ymin = estimate.sd-std.error.sd)) +
  facet_grid(vars(response), vars(trait), labeller = as_labeller(c(traitnames, fitnessnames))) + 
  geom_hline(yintercept=0) + geom_pointrange(size=1) + theme_minimal() +
  scale_color_manual(values = year_pal) + labs(y="SD-standardized slope", x="Year")
```
